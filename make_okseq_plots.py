...
make_heatmaps=True
make_panels_plots=True

pf.base_dir = "/Users/sarahkeegan/okseq_data/strand_bias_plots"
pf.site='TSS' #change to 'TTS' to make same plots around TTS sites
pf.rep_file_name = "rpe_edu_2"

# Load data from table, add columns for gene length and transcriptional volume
df_rpe_edu_2 = pf.load_data()
...
# Plotting C/(C+W) read counts (strand bias) around gene TSS sites, data averaged over all genes
w_data = df_rpe_edu_2.loc[:,'dist_w_100_from_' + pf.site + '_1':'dist_w_100_from_' + pf.site + '_' + str(pf.length_around_site + 1)].mean()
c_data = df_rpe_edu_2.loc[:,'dist_c_100_from_' + pf.site + '_1':'dist_c_100_from_' + pf.site + '_' + str(pf.length_around_site + 1)].mean()
...
if(make_panels_plots):
    ...
    pf.make_panels_plot_with_data([df_rpe_edu_2,], filter_col='', quartile_col='fpkm', filename='Fig2a',
    ...
    pf.make_panels_plot_with_data([df_rpe_edu_2,], filter_col='fpkm', quartile_col='length', filename='Fig2e',
    ...
    pf.make_panels_plot_with_data([df_rpe_edu_2,], filter_col='', quartile_col='volume', filename='Fig2g',
                                line_colors=['black',], ylim=(29,81),yticks=[30, 40, 50, 60, 70, 80],
...
if(make_heatmaps):
    ...
    pf.make_heatmap_plot_with_data([df_rpe_edu_2,], filter_col='fpkm', sort_col='length', color='YlGnBu_r', filename='Fig2f', around_mid=100)

    # 2h Heatmap showing same data as in 2g, above - each row is the s.b. for a gene.  Rows sorted by volume (FPKM*length)
    pf.make_heatmap_plot_with_data([df_rpe_edu_2,], filter_col='', sort_col='volume', color='YlGnBu_r', filename='Fig2g')

# NORM SB plot, and its derivative plot
pf.make_TSS_to_TTS_plots([df_rpe_edu_2,],)

# p-values as in PMID: 30598550, Fig 2g:
ret_vals = pf.calculate_pvalue(data_df1=df_rpe_edu_2, data_df2='', filter_col='', quartile_col='volume', stat_range=[[-50,-30],[1,10]])
...
# *NOTE*: to compare 2 data sets, e.g. HU-treated vs. control, as in PMID: 30598550, Fig. 4a, the function can be called in this manner:
# ret_vals = pf.get_stats(data_df1=df_rpe_edu_2, data_df2=df_rpe_hu_treated, filter_col='', quartile_col='volume', stat_range=[[-50,-30],[1,10]])
